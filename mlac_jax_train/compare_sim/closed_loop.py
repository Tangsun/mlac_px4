...
from window_utils import generate_time_windows
from closed_loop_core import run_smc_window
...
def plot_closed_loop_trajectories(
        ts_axis, measured_states, jax_pos, jax_vel, jax_euler_deg, jax_omega,
        reference_data, attitude_cmd_data, bodyrate_cmd_data, vel_data, output_dir
):
    ...
    t_ref, ref_pos, ref_vel, _, _, _ = reference_data

    # Truncate all to minimum length to ensure plotting consistency
    min_len = min(ts_axis.shape[0], measured_states.shape[0], jax_pos.shape[0], ref_pos.shape[0])
    ...
    ang_meas_deg = interp_series(
        t_ang_meas, np.rad2deg(ang_body_meas) if ang_body_meas.size else np.empty((0, 3)), 3
    )
...
def main():
    ...
    parser.add_argument("--traj-file", help="Reference .npy trajectory.")
    ...
    if args.plot_dir and reference_tuple and reference_tuple[0].size > 0:
        ...
        plot_pose_vs_reference(pose_data, reference_tuple, os.path.join(args.plot_dir, "pose_vs_reference.png"))
        plot_command_streams(bag_data["attitude_cmd"], bag_data["bodyrate_cmd"], os.path.join(args.plot_dir, "command_streams.png"))
    ...
    for w_idx, (start_t, end_t, start_i, end_i) in enumerate(
        ...
        # --- CRASH-PROOF ASSIGNMENT ---
        # Instead of failing on shape mismatch, we take the minimum overlap.
        # This handles cases where sim is shorter than window OR window is shorter than sim.
        
        # 1. Determine safe length
        n_sim = pos_sim.shape[0]
        n_meas = measured_slice.shape[0]
        n_safe = min(n_sim, n_meas)
        
        if n_safe == 0:
            print(f"[Window {w_idx}] Skipped due to empty data.")
            continue

        # 2. Slice both sides to safe length
        pos_sim = pos_sim[:n_safe]
        vel_sim = vel_sim[:n_safe]
        euler_sim_deg = euler_sim_deg[:n_safe]
        omega_sim = omega_sim[:n_safe]
        measured_slice = measured_slice[:n_safe]
        ...
        # 4. Safe array assignment into global buffer
        # This ensures we never write outside the allocated slice
        fill_count = min(n_safe, end_i - start_i)
        pred_pos[start_i : start_i + fill_count] = pos_sim[:fill_count]
        pred_vel[start_i : start_i + fill_count] = vel_sim[:fill_count]
        pred_euler_deg[start_i : start_i + fill_count] = euler_sim_deg[:fill_count]
        pred_omega[start_i : start_i + fill_count] = omega_sim[:fill_count]
        pred_valid[start_i : start_i + fill_count] = True
